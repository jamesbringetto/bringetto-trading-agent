...
def upgrade() -> None:
    # Create enum types
    op.execute("CREATE TYPE strategytype AS ENUM ('orb', 'vwap_reversion', 'momentum_scalp', 'gap_and_go', 'eod_reversal', 'experimental')")
    ...
    op.execute("CREATE TYPE marketregime AS ENUM ('trending_up', 'trending_down', 'range_bound', 'high_volatility', 'low_volatility', 'unknown')")
    ...
    op.create_table(
        ...
        sa.Column("type", postgresql.ENUM("orb", "vwap_reversion", "momentum_scalp", "gap_and_go", "eod_reversal", "experimental", name="strategytype", create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("status", postgresql.ENUM("open", "closed", "cancelled", "partial", name="tradestatus", create_type=False), nullable=False, server_default="open"),
    ...
    op.create_table(
        ...
        sa.Column("timestamp", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("decision_type", postgresql.ENUM("entry", "exit", "hold", "skip", name="decisiontype", create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("variant_a_strategy_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("strategies.id"), nullable=False),
        sa.Column("variant_b_strategy_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("strategies.id"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.CheckConstraint("status IN ('active', 'completed', 'cancelled')", name="check_ab_status"),
    )
    ...
    op.create_table(
        ...
        sa.Column("timestamp", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("symbol", sa.String(10), nullable=False),
        sa.Column("regime_type", postgresql.ENUM("trending_up", "trending_down", "range_bound", "high_volatility", "low_volatility", "unknown", name="marketregime", create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("timestamp", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("severity", postgresql.ENUM("info", "warning", "error", "critical", name="alertseverity", create_type=False), nullable=False),
